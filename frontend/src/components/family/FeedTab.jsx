...
export default function FeedTab({ token }) {
  const { t } = useLanguage();
  const dark = false;
  ...
  const muted = 'text-black/50';
  const body = 'text-[#0a0a0a]';
    ...
    <div onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
      ...
      <ul className="space-y-3">
        {feed.map((item) => (
          <li
            key={item.id}
            className={`overflow-hidden ${glassPanel(dark)} ${item.unread ? 'border-l-[3px]' : ''}`}
            ...
            <div className="p-3.5">
              <div className="flex items-center gap-3 mb-2.5">
                {item.residentPhotoUrl ? (
                  ...
                  <div
                    className="w-11 h-11 rounded-full flex items-center justify-center text-[16px] font-medium"
